...
int main()
{
   ...
   /* for(i=0; i<131; i++)
    ...
    int ok,curent[131],intermediar[131],index_cur=1,index_interm,prima=1;///prima este pur si simplu iteratia pentru prima litera
    curent[0]=0;
    ok=0;
    for(i=0; i<strlen(s); i++)
    {
        ...
        for(j=0; j<133; j++)
            for(k=0; k<10; k++)
            {
                ...
                if(prima==0)
                    if(mat[curent[j]][(int)s[i]][k]!=0)
                        intermediar[index_interm++]=mat[curent[j]][(int)s[i]][k];
            }
